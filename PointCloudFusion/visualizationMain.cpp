...
*/
std::string filePath;
int start(std::string p_filePath )
{
...
osg::ref_ptr<osg::Node> DrawImage(Mat img,int dimFlag,int colorFlag)
{
    ...
    cloud.width    = tempPoint->size();
    cloud.height   = 1;
    cloud.is_dense = false;
    cloud.points.resize (cloud.width * cloud.height);
    for(int i=0;i<tempPoint->size();i++){
        cloud.points[i].x  = tempPoint->at(i).x();
        cloud.points[i].y  = tempPoint->at(i).y();
        cloud.points[i].z  = tempPoint->at(i).z();
        cloud.points[i].r  = temColor->at(i).r();
        cloud.points[i].g  = temColor->at(i).g();
        cloud.points[i].b  = temColor->at(i).b();
        //cloud.points[i].a  = temColor->at(i).a();
    }
...
//        //fileName是文件名
//        qDebug()<<fileName;
    pcl::io::savePCDFileASCII<PointT> (filePath, cloud);
    qDebug()<<"Save "<<QString::fromStdString(filePath)<<" Complete!";
//    }
